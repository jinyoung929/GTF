...
# ───────────────────────────────────────────────────────────────────────────
# §9 라우트 등록 — 엔드포인트 정의는 gtf_app/routers/* 에 있다.
# 이 import는 반드시 파일 맨 끝이어야 한다: 라우터 모듈이 server의 이름들을
# 되가져오므로(순환 import), 모든 정의가 끝난 뒤에만 안전하다.
#
# __main__일 때 건너뛰는 이유: `python server.py`로 직접 실행하면 이 파일이
# __main__으로 한 번, 라우터의 `from server import ...` 때문에 server 모듈로 또 한 번,
# 총 두 번 로드된다. 두 번째 로드가 아직 초기화 중인 라우터 모듈을 다시 등록하려 들어
# 순환 import로 깨진다. 등록은 uvicorn이 import하는 server 모듈 쪽에서만 한다.
# ───────────────────────────────────────────────────────────────────────────
if __name__ != "__main__":
    from gtf_app.routers import register_routers  # noqa: E402

    register_routers(app)


def main() -> None:
    """개발용 직접 실행 진입점. 운영은 Procfile의 `uvicorn server:app`을 쓴다.

    앱을 객체가 아니라 "server:app" 문자열로 넘긴다 — uvicorn이 server를 모듈로
    import하게 해서, 라우트가 등록된 쪽 앱이 서빙되도록 하기 위함이다.
    """
