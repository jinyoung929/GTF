...
VALUES
  ...
  ('inventory', 'A1200', '재고자산', '재고자산', 'judgment', 'K-IFRS 제1002호 문단 25는 후입선출법을 허용하지 않으므로 원가결정방법을 확인하고, 후입선출법 적용 시 선입선출·가중평균 재계산액과의 차이를 조정합니다. 고정제조간접비의 정상조업도 배부도 검토합니다.', true, '1970-01-01T00:00:00Z'),
  ('lease', 'A2100', '리스', '사용권자산', 'judgment', 'K-IFRS 제1116호 측정을 위해 리스기간, 지급액, 선택권, 할인율을 추가 확인합니다. 수정소급법에 따라 사용권자산과 동액의 리스부채 분개가 쌍으로 생성됩니다.', true, '1970-01-01T00:00:00Z'),
  ...
  ('goodwill', 'A3200', '영업권', '영업권(상각 중지)', 'judgment', 'K-IFRS 제1103호는 영업권 상각을 금지하므로 최초채택 시 K-GAAP 누적 상각비를 소급 환입해 취득원가로 복원하고, 제1036호에 따라 매년 손상검사를 수행합니다.', true, '1970-01-01T00:00:00Z'),
  ...
  ('compound_instrument', 'L2500', '복합금융상품', '부채·자본 분리 검토(전문가 평가 필요)', 'judgment', 'K-IFRS 제1032호에 따라 전환사채 등 복합금융상품은 부채요소와 자본요소(또는 내재파생상품)를 분리합니다. 공정가치 평가가 필요하므로 도구는 식별과 근거 제시까지만 수행합니다.', true, '1970-01-01T00:00:00Z'),
  ('derivative', 'F2000', '파생상품', '공정가치 측정(전문가 평가 필요)', 'judgment', 'K-IFRS 제1109호에 따라 모든 파생상품은 공정가치로 인식합니다. 평가와 위험회피 문서화는 전문가 영역이므로 도구는 식별과 근거 제시까지만 수행합니다.', true, '1970-01-01T00:00:00Z'),
