...
현재 공개 MVP는 실제 고객 운영 데이터가 아니라 재구현된 포트폴리오 버전이므로, 실무 성과와 구현 검증 지표를 구분해 기록합니다.

| 구분 | 지표 | 현재 수치 |
| --- | --- | --- |
| 입력 채널 | 수동 입력, CSV, Excel, PDF 표 추출, 이미지/OCR 경로 | 5개 입력 경로 |
| 파일 포맷 | 로컬 파서 또는 OCR 경로로 처리하는 업로드 형식 | PDF, Excel, CSV, 이미지 4종 |
| 계정 매핑 범위 | K-GAAP/영문 계정명 별칭을 내부 표준계정코드로 정규화 | 38개 별칭 -> 9개 표준계정 |
| IFRS 표시 매핑 | 내부 표준계정에서 IFRS 표시 라인으로 연결 | 9개 IFRS 표시 라인 |
| 기준서 커버리지 | 표시·인식·측정 판단에 연결한 K-IFRS 기준 | 제1002호, 제1007호, 제1037호, 제1038호, 제1109호, 제1115호, 제1116호 등 7개 |
| 판단 필요 항목 | 리스, 개발비, 수익, 금융상품, 충당부채, 매출채권, 미분류 계정 체크리스트 | 7개 영역 / 21개 입력 항목 |
| 데이터 모델 | 사용자, 세션, 프로젝트, 업로드, 추출, 변환, 리뷰, 감사 로그, 기준정보 테이블 | 16개 테이블 |
| API 범위 | 인증, 프로젝트, DART 가져오기, 업로드, 추출, 검증, 변환, 리뷰, 감사 로그, 내보내기 엔드포인트 | 19개 핵심 엔드포인트 |
| 검증 룰 | 누락 금액, 중복 계정, 미분류, 기간 불일치, 단위 오류, 음수 금액, 재무상태표 균형 등 | 12개 검증 체크 |
| 조정분개 처리 | 변환 대상 계정 행마다 조정분개 후보와 계산 근거 생성 | 입력 계정 행 기준 1:1 생성 |
| 로컬 처리량 벤치마크 | 120개 샘플 계정 행 변환 기준 | 조정분개 후보 120건 / 판단 항목 84건 / 주석 초안 84건 |
| 조정분개 정확도 검증 | 리스 현재가치, 개발비 자산화, 충당부채 인식, 수익인식, 단순/판단 매핑 테스트 | 자동화 테스트 24개 |
| 계산 허용 오차 | IFRS 16 리스료 현재가치 계산을 독립 할인 현금흐름 합산값과 비교 | 0.01원 단위 허용 오차 |
| 리뷰 산출물 | 조정분개 CSV, 판단 근거 리포트, 검토용 Excel Workbook, 주석 초안, 감사 로그 제공 | 5종 산출물 |
| 리뷰 흐름 단축 | 업로드, 계정 반영, 변환 생성, 검토 기록을 한 화면에서 처리 | 수기 취합 중심 4단계 -> 화면 내 1개 흐름 |
...
SQLite 초기 스키마는 `sqlite/schema.sql`에 분리되어 있고, 서버 시작 시 해당 파일을 적용한 뒤 필요한 호환 마이그레이션과 기준정보 시드를 실행합니다.

운영 배포에서는 Neon 등 관리형 Postgres 사용을 권장합니다.

## 코드 구조

빌드 스텝 없이 단일 Python 프로세스로 배포하는 단순함은 유지하되, 핵심 책임은 다음처럼 분리합니다.

- `server.py`: 실행 진입점, DB 연결/초기화, 파일 업로드/OCR/AI 같은 I/O, HTTP 핸들러 조립
- `gtf_app/domain.py`: 계정 정규화, 매핑, 검증, IFRS 변환 계산, CSV/검토 리포트 생성
- `gtf_app/routing.py`: 공개 경로와 HTTP 메서드별 라우트 매칭
- `gtf_app/assets.py`: 빌드 없는 배포를 위한 HTML/CSS/브라우저 JS 애셋
- `sqlite/schema.sql`, `postgres/schema.sql`: 로컬/운영 데이터베이스 스키마

이 구조는 프론트 번들러나 웹 프레임워크를 추가하지 않는 대신, 회계 도메인 로직과 라우팅/표현 계층이 한 파일에 섞이지 않도록 하는 MVP용 절충안입니다.
...
테스트는 회계 변환 계산뿐 아니라 로그인 성공/실패, 데모 계정 읽기 전용 플래그, 읽기 전용 사용자의 프로젝트 생성 차단, SQLite 스키마 분리 로딩을 검증합니다. 포트 바인딩 없이 핸들러를 직접 호출하므로 로컬과 CI에서 동일하게 실행됩니다.
